...
const buildLongFormPrompt = require("../ai/prompts/script-longform");
const buildShortFormPrompt = require("../ai/prompts/script-shortform");
const buildHumanStorytellingEngine = require("../ai/prompts/script-storytelling");
...
router.post("/api/script/generate", requireApiAuth, async (req, res) => {
  ...
  // Handle Storytelling mode using Groq API
  if (type === "storytelling") {
    if (!GROQ_API_KEY) {
      console.error("GROQ_API_KEY is missing in environment variables for Story Mode");
      return res.status(500).json({ success: false, error: "Server configuration error: Groq API key is missing." });
    }
    ...
    try {
      ...
      console.error("Story Mode Groq Generation Error:", errMsg);
  ...
  let bodyText = "";
  let ctaText = "";
  let originalHookMissing = false;
  let originalCtaMissing = false;

  if (type === "short") {
    // Highly robust parser to extract hook, body, cta
    const extractShortForm = (text) => {
      let regex = /\[(?:HOOK|BODY|CTA)\]/gi;
      let matches = [];
      let match;
      while ((match = regex.exec(text)) !== null) {
        const tag = match[0].toUpperCase();
        let label = "hook";
        if (tag.includes("BODY")) label = "body";
        if (tag.includes("CTA")) label = "cta";
        matches.push({ index: match.index, label, length: match[0].length });
      }

      // Fallback: search for markdown headers or bold lines
      if (matches.length === 0) {
        regex = /(?:\*\*|###|##|#)?\s*\b(HOOK|BODY|CTA)\b\s*(?:\*\*|:)?/gi;
        while ((match = regex.exec(text)) !== null) {
      ...
      const res = { hook: "", body: "", cta: "" };
      if (matches.length === 0) {
        // Fallback split by paragraphs
        const paras = text.split(/\n\n+/).map(p => p.trim()).filter(Boolean);
        if (paras.length >= 3) {
          res.hook = paras[0];
          res.cta = paras[paras.length - 1];
          res.body = paras.slice(1, paras.length - 1).join("\n\n");
        } else {
          res.body = text;
        }
      } else {
        ...
        for (let i = 0; i < matches.length; i++) {
          ...
          res[matches[i].label] = text.substring(start, end).trim();
        }
      }
      return res;
    };

    const parsed = extractShortForm(scriptText);
    hookText = parsed.hook;
    bodyText = parsed.body;
    ctaText = parsed.cta;

    // VALIDATION & FALLBACK SYSTEM
    const cleanSub = cleanTopic(topic);
    if (!hookText || hookText.trim().length < 5) {
      originalHookMissing = true;
      console.warn("AI failed to return HOOK. Injected fallback hook.");
      hookText = `Most people get ${cleanSub} wrong in the first 3 seconds.`;
    }
    if (!ctaText || ctaText.trim().length < 5) {
      originalCtaMissing = true;
      console.warn("AI failed to return CTA. Injected fallback CTA.");
      ctaText = `If you want to master ${cleanSub}, follow for more daily value.`;
    }
    if (!bodyText || bodyText.trim().length < 5) {
      bodyText = scriptText.replace(/\[HOOK\]|\[BODY\]|\[CTA\]/gi, "").trim() || `Here is the real breakdown of how ${cleanSub} works. It's not about complex tools, it's about consistency.`;
    }

    // Reconstruct normalized scriptText
    scriptText = `[HOOK]\n${hookText}\n\n[BODY]\n${bodyText}\n\n[CTA]\n${ctaText}`;
  }
  ...
  if (type === "long") {
    ...
    if (originalHookMissing || originalCtaMissing) {
      sectionsMatch = false;
    }
  }
  ...
  return res.json({
    success: true,
    script: scriptText,
    hook: type === "short" ? hookText : undefined,
    body: type === "short" ? bodyText : undefined,
    cta: type === "short" ? ctaText : undefined,
    metadata: {
      type,
      wordCount,
      estimatedDuration,
